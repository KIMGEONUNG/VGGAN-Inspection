...
if __name__ == "__main__":
    ...
    domain = np.arange(0, 256, 2)
    grid = np.meshgrid(domain, domain, domain)
    batch = 4096
    colors = np.stack(grid, axis=-1).reshape(-1, batch, 3)
    ...
    for color in tqdm(colors):
        ...
        hint = torch.Tensor(color).div(255).mul(2).add(-1).cuda()
        hint_embd = model.color2embd(hint)
        quant, emb_loss, info = model.quantize(hint_embd)
